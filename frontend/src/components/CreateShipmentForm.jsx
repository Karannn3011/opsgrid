...
const CreateShipmentForm = ({ onShipmentCreated, onCancel }) => {
  ...
  const [isTruckSelectionDisabled, setIsTruckSelectionDisabled] = useState(false);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setAvailableTrucks(trucksRes.data.filter(t => t.status === 'WORKING'));
      } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div className="space-y-4">
        <input name="description" placeholder="Shipment Description" required onChange={handleChange} className="mt-1 block w-full rounded-md dark:bg-gray-700 dark:text-white" />
        <div className="grid grid-cols-2 gap-4">
          <input name="origin" placeholder="Origin" required onChange={handleChange} className="mt-1 block w-full rounded-md dark:bg-gray-700 dark:text-white" />
          <input name="destination" placeholder="Destination" required onChange={handleChange} className="mt-1 block w-full rounded-md dark:bg-gray-700 dark:text-white" />
        </div>
        
        {/* Updated Driver Select with new onChange handler */}
        <select name="assignedDriverId" required onChange={handleDriverChange} value={formData.assignedDriverId} className="mt-1 block w-full rounded-md dark:bg-gray-700 dark:text-white">
          <option value="">Assign a Driver</option>
          {availableDrivers.map(d => <option key={d.userId} value={d.userId}>{d.fullName}</option>)}
        </select>
        ...
        >
          ...
          {availableTrucks.map(t => <option key={t.id} value={t.id}>{t.licensePlate} ({t.make})</option>)}
        </select>
      </div>
      <div className="mt-6 flex justify-end space-x-3">
        <button type="button" onClick={onCancel} className="rounded-md bg-gray-200 px-4 py-2">Cancel</button>
        <button type="submit" disabled={submitting} className="rounded-md bg-blue-600 px-4 py-2 text-white disabled:opacity-50">
